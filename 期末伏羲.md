...
<img src="https://raw.githubusercontent.com/shuixianhua-ai/demo/main/image/image-20210619210546196.png" alt="image-20210619210546196" style="zoom:33%;" />
...
#### Architectural Description L**anguage (**ADL): <u>UML</u>
...
* Scenario-Based Test Design【基于场景的测试设计，基于故障的测试忽略了不正确的规格说明盒子系统间的交互两种错误】：集中于<u>用户做什么</u>，而不是产品做什么。这意味着捕获用户必须执行的任务（通过用例），然后将它们及其变体应用作为测试。更加贴近实际。
...
* mobile page compliance checkers 移动式页面合规性检查程序
...
* internet of things【物联网】
  * 日常对象交流和报告有关其用户及其环境的信息的能力，如果安全性出现问题，则将会有大问题
...
* security requirements elicitation【引出】
  ...
  * 首先找到可识别解决系统安全缺陷的解决方案的Locate attack pattern【攻击模式】
...
* security model
  ...
  * 每一状态必须包含和安全性相关的信息，state model【状态模型】可以帮助软件工程师确保系统允许的一系列状态转换开始和结束处于安全状态
  * 

* measure design
  * 安全性软件的属性
    * sysytem dependence【系统可靠性，软件可以在不友好的环境下运行】，
    * trustworthiness【可信性，系统不会在恶意的方式中进行】，
    * survivability【生存性，在已妥协的情况下系统仍可能继续运行】
  * 安全性度量要以测度【measure】为基础，
    * asset value【资产价值】，
    * threat likelihood【威胁似然性测度】
    * 系统漏洞测度
  
* correctness check【正确性检查，贯穿整个软件开发周期】
  * 从系统攻击的角度来分析，对于资产的显露度应该在开发早期就确定；
  * 而由系统用例导出的威胁模型对于风险缓解，监测和管理的安全性部分做出了解释
  * 在建模和构件活动中，
    * 质量保证活动应该包括安全性标准的制定和安全性指南的开发
    * 软件验证活动应确保安全性测试用例是完备的，可以追溯到系统的安全性需求
  * 许多这类的安全性检查应该包括在常规软件工程任务的**审核audit，审查inspections和测试testing**活动中，在此期间收集的数据被分析和总结为一个安全案例

#### **Security** **Assurance**【安全性保证】

* 用于显示您已创建了一个安全的产品，以激发最终用户和利益相关者之间的信心
* 其中验证是保证任务的一部分，它提供证据以表明利益相关者可以确信这个软件是符合要求的
* 其中保证案例【又叫安全性用例】是已经讨论过，审核过的材料，支持软件可以满足正在维权的索赔
* 安全性用例【security case elements】
  * security claims【安全索赔】
  * arguments linking claim to each other【通过证据和假设使多个索赔要求之间必须相连的论点】
  * evidence（review，proof）supporting arguments【支持论点的证据和明确假设】
...
过程：版本控制，变更控制，影像管理，配置审核，状态报告，对象标识

软件配置用于标识变更，控制变更，保证恰当的实施**变更**，向其他可能的相关人员报告变更
...
* 存储所有相关配置对象的项目数据库【中央存储库】
* 存储配置对象所有版本的版本管理功能
* 使软件工程师能收集所有配置对象和构造软件特定版本的制作功能
* 版本控制和变更控制系统通常还有问题跟踪【错误跟踪】
...
* 正确性correvtness【软件完成所要求的功能的程度】
* 可维护性maintainability【遇到错误时，程序能被修改的容易程度】
* 完整性integrity【一个系统对安全性攻击【偶然/蓄意】的抵抗能力】
* 可用性usability【对使用的容易程度进行量化】
...
* COCOMO (COnstructive COst MOdel)
  ...
  <img src="https://raw.githubusercontent.com/shuixianhua-ai/demo/main/image/image-20210619115349346.png" alt="image-20210619115349346" style="zoom: 67%;" />
...
* <u>**早在项目进展15%的时候就提供了精确且可靠的性能数据**</u>
...
![image-20210619144138849](https://raw.githubusercontent.com/shuixianhua-ai/demo/main/image/image-20210619144138849.png)
