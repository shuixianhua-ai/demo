...
* 通用的软件模式元素包括：
  ...
  * intent【意义，驱动力】 
  * 类型【包括】
    * task pattern（定义任务，如收集需求） 【重视目的】
    * stage pattern（步骤模式，定义了与框架活动有关的活动，如communication作为一个步骤模式可能包括需求收集等任务） 【重视途径】
    * phase pattern（定义框架活动流程，如螺旋模型和原型开发）
  * 启动条件
  * 问题
  * 解决方案
  * 结果
...
* speculation【思考】
  * 自适应周期计划任务声明项目限制基本要求时间箱发布计划
* collaboration【协作】
  * 收集联合应用程序开发小规格的需求
* learning【学习】
  * 组件实施/测试的焦点小组反馈正式技术审查事后调查结果
